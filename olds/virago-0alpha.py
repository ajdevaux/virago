...
def processed_image_viewer(image, particle_df, cy, cx, rad, pix_per_micron,
    ...
    scale_micron = 15
    scalebar_len_pix = int(round(pix_per_micron * scale_micron,0))
    ...
    def draw_sq(cx,cy):
        sq = plt.Rectangle((cx,cy), scalebar_len_pix, scalebar_len_pix,
                            fill = False, ec = 'red', lw = 3)
        axes.add_patch(sq)

    draw_sq(cx, cy)
    draw_sq(cx - scalebar_len_pix*1.5, cy + scalebar_len_pix*1.5)
    draw_sq(cx - scalebar_len_pix*1.5, cy - scalebar_len_pix*1.5)
    draw_sq(cx + scalebar_len_pix*1.5, cy - scalebar_len_pix*1.5)
    draw_sq(cx + scalebar_len_pix*1.5, cy + scalebar_len_pix*1.5)
    ...
    #                                               linewidth = 2, alpha = 0, stacked = True,
    #                                               color = hist_color,
    #                                               label = z_list)
...
if pgm_toggle.lower() in ('yes', 'y'):
    ...
    while spot_to_scan <= spot_counter:
        ...
        for x in scan_range:
            ...
            vis_blobs = blob_detect_3D(pic3D_masked, min_sig = 1.5, max_sig = 5, thresh = 1)
...
#---------------------------------------------------------------------------------------------#
            if fluor_files:
                ...
                processed_image_viewer(fluor3D_rescale[0], fluor_df,
                                        cy, cx, rad, pix_per_micron, cmap = 'plasma')
...
#---------------------------------------------------------------------------------------------#
        ####Processed Image Renderer
            ...
            processed_image_viewer(pic_to_show, particle_df, cy, cx, rad, pix_per_micron)
#---------------------------------------------------------------------------------------------#
