...
def IRISpgm_scanner(mirror_file, scan_list, image_detail_toggle):
    ...
    try: mirror_file
    except NameError: mirror_toggle = False; print("Mirror file absent")
    else:
        mirror_toggle = True
...
#---------------------------------------------------------------------------------------------#
        ...
        if zslice == 1:
            ...
            #pic_selem = filters.rank.equalize(pic_rescale, selem = morphology.disk(50))
...
#---------------------------------------------------------------------------------------------#
        def particle_quant(image, d_blobs, particle_array, zslice, sdm_filter):
            ...
            print("Image scanned: " + str(pgmfile) + "\n")
...
#---------------------------------------------------------------------------------------------#
    def duplicate_remover(DFrame):
        ...
        DFrame.sort_values(['pc'], kind = 'quicksort', inplace = True)
        DFrame.drop_duplicates(subset = ('yx_5'), keep = 'last', inplace = True)
        DFrame.drop_duplicates(subset = ('yx_10'), keep = 'last', inplace = True)
        DFrame.drop_duplicates(subset = ('yx_10/5'), keep = 'last', inplace = True)
        DFrame.drop_duplicates(subset = ('yx_5/10'), keep = 'last', inplace = True)
        DFrame.drop_duplicates(subset = ('yx_ceil'), keep = 'last', inplace = True)
        DFrame.drop_duplicates(subset = ('yx_floor'), keep = 'last', inplace = True)
    ...
    total_part_df = duplicate_remover(total_part_df)

    total_part_df.to_csv('../virago_output/' + chip_name + '/' + png + '.'
...
#---------------------------------------------------------------------------------------------#
    ...
    plt.savefig('../virago_output/' + chip_name + '/' + png +'.png', dpi = dpi)
...
if pgm_list:
    ...
    if pgmer_toggle.lower() in ('yes', 'y'):
        ...
        while spot <= spot_counter:
            ...
            #     scan_list = [file for file in pgm_list if file.startswith(pass_per_spot_list[scan - 1])]
        ...
        #     for pgmfile in pgm_list:
        #         pgm_name = pgmfile.split(".")
        #         scan_no = int(pgm_name[2])
        #         spot_no = int(pgm_name[1])
        #         png = '.'.join(pgm_name[:3])
        #         if (spot_no == spot) & (scan_no == scan): scan_list.append(pgmfile)
        #         if not scan_list:
        #             print("Missing pgm files... moving to next set")
        #             scan += 1
        #             if scan > pass_counter:
        #                 scan = 1
        #                 spot += 1
        #         if len(scan_list) == zslice_count:
        ...
        # normalized_density_vir = density_normalizer(spot_data_vir, pass_counter, spot_list)
...
if len(vir_csv_list) == (len(pgm_list) / zslice_count):
    ...
    if csv_reader_toggle in ('yes', 'y'):
        ...
        area_list = np.array([(float(csvfile.split(".")[5]) / 1e6) for csvfile in vir_csv_list])
