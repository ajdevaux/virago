...
def load_image2(pic_dir, data_select):
    os.chdir(pic_dir)
    ...
    img_final = measure.block_reduce(pic,block_size = (2,2))

    return img_final, data_name
...
img_final, data_name = load_image2(pic_dir, data_select)
...
p = figure(plot_width = ncols, plot_height = nrows, min_border=10, min_border_left=5,
          x_range=(0,ncols), y_range=(0,nrows), x_axis_location=None, y_axis_location=None,
          tools = ['box_zoom,box_select,lasso_select,tap', particle_data, "reset"],
          title = data_name)
p.image(image=[img_final], x=-0.5, y=0, dw=(ncols - 0.5), dh=nrows)
...
ph = figure(plot_width=p.plot_width, plot_height=180,
            ...
            tools = ['box_select,xbox_zoom,reset',histo_data,'tap'], toolbar_location = 'right',
...
def img_change(attrname, old, new):
    ...
    os.chdir(expt_dir)
    pic_dir = '../virago_output/' + chip_name + '/processed_images'
    new_pic, data_name = load_image2(pic_dir, new_select)
